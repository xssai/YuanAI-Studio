 ...
 * 表格时间格式化
 */
export function format_date(cellValue) {
  if (cellValue == null || cellValue == "") return "";
  var date = new Date(cellValue)
  var year = date.getFullYear()
  var month = date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1
...
export function isBrowserChrome() {
  ...
  const isChrome = userAgent.indexOf("Chrome") !== -1 && userAgent.indexOf("Safari") !== -1 && userAgent.indexOf("Edg") === -1
  return isChrome
}


// 数值转万单位
export function numberToWan(number) {
  let result = 0
  if (number >= 10000) {
    const newNumner = (number / 10000).toFixed(2)
    result = newNumner + '万'
  } else {
    result = number
  }
...
// 分钟与当前时间差计算
export function calculateMinutesDifference(time) {
 ...
 * @returns {Object}
 */
export function vuetifyFileInfo(file) {
  return new Promise((resolve, reject) => {
    if (typeof file === 'object') {
      const { name, size, type, lastModifiedDate, lastModified } = file
      const fileName = name.substring(0, name.indexOf('.'))
      const expandName = name.substring(name.indexOf('.') + 1, name.length).toLowerCase()
      const fileData = { completeName: name, fileName, expandName, fileSize: size }
      resolve(fileData)
    } else {
      reject("fileError")
    }
...
// 十位时间戳转换 MM-DD HH:mm
export function dataFmt(time) {
  var cellValue = time * 1000
  if (cellValue == null || cellValue == "") return "";
  var date = new Date(cellValue)
  var year = date.getFullYear()
  var month = date.getMonth() + 1 < 10 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1
